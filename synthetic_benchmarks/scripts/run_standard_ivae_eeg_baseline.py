...
-----------------------
Run this from the root of your SID-Net / protocol_sidnet project and make sure the original
iVAE repo is importable, so that `from models import cleanIVAE` works. If it is not importable,
pass --ivae-root /path/to/ivae_repo .

Example
-------
python scripts/run_standard_ivae_eeg_baseline.py \
  --config configs/sub01_sub02_cross_sub_ses.yaml \
  --dataset-root /path/to/InnerSpeech2021 \
  --ivae-root /path/to/ivae_repo \
  --protocol both \
  --latent-dim 32 \
  --u-segments 20 \
  --ivae-epochs 120 \
...
def _import_clean_ivae(ivae_root: Optional[str | Path]):
    _add_path(ivae_root)
    try:
        ...
        raise ImportError(
            ...
            "or run this script from an environment where `from models import cleanIVAE` works."
        ) from exc
    return cleanIVAE
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--ivae-root", type=str, default=None, help="Path to original iVAE repo containing models.cleanIVAE, if not already importable.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    cleanIVAE_cls = _import_clean_ivae(args.ivae_root)
