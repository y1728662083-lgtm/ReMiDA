...
def to_one_hot(x, m=None):
    ...
    for i, xi in enumerate(x):
        ...
        # np.int is deprecated in recent NumPy versions; use built-in int.
...
class CustomSyntheticDataset(Dataset):
    def __init__(self, X, U, S=None, device='cpu', U_raw=None, u_raw_id=None,
                 domain_id=None, subject_id=None, session_id=None):
        self.device = device
        ...
        if S is not None:
            ...
            self.s = self.x
        # Optional fields (set by runners for drift experiments)
        #   - self.z / self.domain_id: per-sample domain/session id (np.ndarray or list)
        #   - self.subject_id, self.session_id: hierarchical subject/session ids
        #   - self.u_raw_id: raw shared condition id (e.g., time-bin)
        #   - self.s_anchor: per-sample ICA anchor in a canonical order (torch.Tensor)
        self.z = None if domain_id is None else np.asarray(domain_id).astype(np.int64)
        self.domain_id = self.z
        self.subject_id = None if subject_id is None else np.asarray(subject_id).astype(np.int64)
        self.session_id = None if session_id is None else np.asarray(session_id).astype(np.int64)
        self.u_raw = None if U_raw is None else torch.from_numpy(U_raw).to(self.device)
        if u_raw_id is None:
            if U_raw is not None:
                U_raw_np = np.asarray(U_raw)
                if U_raw_np.ndim == 2 and U_raw_np.shape[1] > 1:
                    self.u_raw_id = U_raw_np.argmax(axis=1).astype(np.int64)
                else:
                    self.u_raw_id = U_raw_np.reshape(-1).astype(np.int64)
            else:
                self.u_raw_id = None
        else:
            self.u_raw_id = np.asarray(u_raw_id).astype(np.int64)
        self.s_anchor = None
        self.len = self.x.shape[0]
        self.latent_dim = self.s.shape[1]
        self.aux_dim = self.u.shape[1]
        self.data_dim = self.x.shape[1]
        self.nps = int(self.len / self.aux_dim) if self.aux_dim > 0 else self.len
    ...
    def __getitem__(self, index):
        out = [self.x[index], self.u[index], self.s[index]]

        if self.z is not None:
            z_val = int(self.z[index])
            out.append(torch.tensor(z_val, dtype=torch.long))

        if self.s_anchor is not None:
            out.append(self.s_anchor[index])

        if self.u_raw_id is not None:
            out.append(torch.tensor(int(self.u_raw_id[index]), dtype=torch.long))
        if self.subject_id is not None:
            out.append(torch.tensor(int(self.subject_id[index]), dtype=torch.long))
        if self.session_id is not None:
            out.append(torch.tensor(int(self.session_id[index]), dtype=torch.long))
    ...
    def get_metadata(self):
        ...
        if self.z is not None:
            meta['n_domains'] = int(np.max(self.z)) + 1
        if self.subject_id is not None:
            meta['n_subjects'] = int(np.max(self.subject_id)) + 1
        if self.session_id is not None:
            meta['n_sessions'] = int(np.max(self.session_id)) + 1
        return meta
